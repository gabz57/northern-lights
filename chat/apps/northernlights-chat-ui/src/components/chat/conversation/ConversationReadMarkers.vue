<template>
  <div class="conversation-read-markers">
    <div
      v-for="[topPosition, localStack] in stackedLocalMarkers"
      :key="topPosition"
    >
      <ConversationReadMarker
        class="conversation-read-markers__marker"
        v-for="positionedReadMarker in localStack"
        :key="positionedReadMarker.chatterId"
        :style="{
          top: `${positionedReadMarker.topPosition}px`,
          right: `${positionedReadMarker.rightPosition}px`,
        }"
...
import type {
  PositionedReadMarker,
  PositionedReadMarkerWithXPos,
} from "@/composables/use-conversation-read-markers";
...
export default defineComponent({
  ...
  setup(props) {
    ...
    const stackedLocalMarkers = ref<
      Map<number, PositionedReadMarkerWithXPos[]>
    >(new Map<number, PositionedReadMarkerWithXPos[]>());
    const timelinePerChatterId = ref<Map<ChatterId, gsap.core.Timeline>>(
      new Map<ChatterId, gsap.core.Timeline>()
    );

    const MOVE_DOWN_DURATION = 0.5;
    const MOVE_LEFT_DURATION = 0.05;
    ...
    const unstack = (localMarker: PositionedReadMarker) => {
      Array.from(stackedLocalMarkers.value.values())
        .filter((localMarkersStack) =>
          localMarkersStack.some(
            (localMarkersElem) =>
              localMarkersElem.chatterId === localMarker.chatterId
          )
        )
        .forEach((localStack) => {
          const idxToRemove = localStack.findIndex(
            (elem) => elem.chatterId === localMarker.chatterId
          );
          if (idxToRemove > -1) {
            localStack.splice(idxToRemove, 1);
            for (
              let offset = idxToRemove;
              ...
              gsap.to(localStack[offset], {
                ...
                rightPosition:
                  DEFAULT_RIGHT_POSITION_IN_PX + offset * MARKER_SPACING_IN_PX,
              });
            }
          }
        });
    ...
    const stack = (localMarker: PositionedReadMarkerWithXPos) => {
      const idxToRemove = localMarkers.value.findIndex(
        (elem) => elem.chatterId === localMarker.chatterId
      );
    ...
    const updateLocalMarker = (
      newMarker: PositionedReadMarker,
      oldMarker: PositionedReadMarker | undefined
    ) => {
      ...
      timelinePerChatterId.value.set(
        newMarker.chatterId,
        gsap
          .timeline({
          ...
          .to(localMarker, {
            duration: MOVE_DOWN_DURATION,
            topPosition: newMarker.topPosition,
          })
      );
    };
    ...
    watch(markers, (newMarkers, prevMarkers) => {
      if (newMarkers.length === 0) {
        ...
        newMarkers.forEach((newMarker) =>
          updateLocalMarker(newMarker, undefined)
        );
      } else {
        const markerWithTopPositionChange = (
          prevMarker: PositionedReadMarker,
          newMarker: PositionedReadMarker
        ) =>
        ...
        newMarkers
          .filter(
            (newMarker) =>
              prevMarkers.find((prevMarker) =>
                markerWithTopPositionChange(prevMarker, newMarker)
              ) ||
              !prevMarkers.find(
                (prevMarker) => prevMarker.chatterId === newMarker.chatterId
              )
          )
          .forEach((newMarker) =>
            updateLocalMarker(
        ...
        prevMarkers
          .filter(
            (prevMarker) =>
              newMarkers.find(
                (newMarker) => prevMarker.chatterId === newMarker.chatterId
              ) === undefined
          )
