<template>
  <div class="conversation-creation">
    ...
    <button
      ...
      :disabled="
        newConversationName.length === 0 || selectedChatters.length === 0
      "
    >
      CREATE
    </button>
    <div>
      <div>Chatters:</div>
      <ul>
        <li v-for="chatter in selectedChatters" :key="chatter.id">
          {{ chatter.name }}
        </li>
      </ul>
    </div>
...
export default defineComponent({
  ...
  setup() {
    ...
    watch(
      [newChatterName, selectedChatters],
      ([chatterName]) => {
        const notCurrentChatter = (
          chatter: Chatter,
          currentChatterId: ChatterId
        ) => chatter.id !== currentChatterId;
        const notAlreadyAdded = (chatter: Chatter, chatters: Chatter[]) =>
          !chatters.find(
            (selectedChatter) => chatter.id === selectedChatter.id
          );
        ...
        if (chatterName.length === 0) {
          ...
          selectableChatters.value = nextSelectableChatters
            ...
            .filter((chatter) =>
              notAlreadyAdded(chatter, selectedChatters.value)
            );
        }
      },
      {
