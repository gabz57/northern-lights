...
import {
    ...
    ConversationData, ConversationDataId,
    ConversationId, ConversationMessageData,
    ConversationPart,
    Conversations, ReadMarkers
} from "@/domain/model";
...
export const useConversationsStore = defineStore("conversations", () => {
    ...
    // state
    const conversations = ref<Conversations>(new Map<ConversationId, Conversation>())
    ...
    const getPrivateConversationWithChatterId = computed(() => (chatterId: ChatterId) => {
    ...
    function updateConversationAddMessage(conversationId: ConversationId, conversationMessageData: ConversationMessageData) {
        conversations.value.get(conversationId)?.data.push(conversationMessageData)
    }

    function updateConversationAddChatter(conversationId: ConversationId, conversationChatterData: ConversationChatterData) {
        conversations.value.get(conversationId)?.participants.push(conversationChatterData.chatterId)
        conversations.value.get(conversationId)?.data.push(conversationChatterData)
    }

    function updateConversationMarkerAsRead(conversationId: ConversationId, readMarkers: ReadMarkers) {
        readMarkers.forEach((conversationDataId, chatterId) =>
            conversations.value.get(conversationId)?.readMarkers.set(chatterId, conversationDataId))
    }

    async function sendMessage(chatterId: ChatterId, conversationId: ConversationId, message: string) {
        await chatApiClient.sendMessage(chatterId, conversationId, message)
    }

    async function markAsRead(chatterId: ChatterId, conversationId: ConversationId, conversationDataId: ConversationDataId) {
        await chatApiClient.markAsRead(chatterId, conversationId, conversationDataId)
    }

    async function createConversation(chatterId: ChatterId, name: string, participants: ChatterId[], dialogue: boolean) {
        await chatApiClient.createConversation(chatterId, name, participants, dialogue)
    }

    async function inviteChatter(chatterId: ChatterId, conversationId: ConversationId, invitedChatterId: ChatterId) {
